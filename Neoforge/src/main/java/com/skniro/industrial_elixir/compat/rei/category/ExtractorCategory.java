...
import com.skniro.industrial_elixir.compat.rei.display.ExtractorDisplay;
...
public class ExtractorCategory implements DisplayCategory<ExtractorDisplay> {
    public static final Identifier TEXTURE = Helper.id(
            "textures/gui/container/machine/extractor.png");
    ...
    public CategoryIdentifier<? extends ExtractorDisplay> getCategoryIdentifier() {
        return ExtractorDisplay.Extractor;
    }
    ...
    public Renderer getIcon() {
        return EntryStacks.of(GrowableOresBlocks.Extractor_Block.get().asItem().getDefaultInstance());
    }
    ...
    public List<Widget> setupDisplay(ExtractorDisplay display, Rectangle bounds) {
        ...
        widgets.add(Widgets.createSlot(new Point(startPoint.x + 100, startPoint.y + 33)).entries(
        ...
        widgets.add(Widgets.createDrawableWidget((context, mouseX, mouseY, delta) -> {
            ...
            context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, startPoint.x + 75, startPoint.y + 35, 190, 0, progress, 14, 256, 256);
        }));
