...
import com.skniro.industrial_elixir.compat.rei.display.MolecularTransformerDisplay;
...
public class MolecularTransformerCategory implements DisplayCategory<MolecularTransformerDisplay> {
    public static final Identifier TEXTURE = Helper.id(
            "textures/gui/container/machine/molecular_transformer.png");
    ...
    public CategoryIdentifier<? extends MolecularTransformerDisplay> getCategoryIdentifier() {
        return MolecularTransformerDisplay.MolecularTransformer;
    }
    ...
    public Renderer getIcon() {
        return EntryStacks.of(GrowableOresBlocks.MolecularTransformerBlock.get().asItem().getDefaultInstance());
    }

    @Override
    public List<Widget> setupDisplay(MolecularTransformerDisplay display, Rectangle bounds) {
        ...
        widgets.add(Widgets.createDrawableWidget((context, mouseX, mouseY, delta) -> {
            long energy = display.getEnergy();
                var textRenderer = Minecraft.getInstance().font;
                String text = String.valueOf("Energy:" + energy);
            int x = startPoint.x + 72 + 16 - textRenderer.width(text) - 1;
            int y = startPoint.y + 53 + 16 - 8 - 1;
                context.drawString(textRenderer, text, x, y, -1, true);
        }));
        return widgets;
    }
