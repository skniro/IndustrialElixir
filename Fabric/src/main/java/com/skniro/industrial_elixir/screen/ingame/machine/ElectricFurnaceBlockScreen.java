...
public class ElectricFurnaceBlockScreen extends AbstractContainerScreen<ElectricFurnaceScreenHandler> {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath(IndustrialElixir.MOD_ID, "textures/gui/container/machine/electric_furnace.png");

    public ElectricFurnaceBlockScreen(ElectricFurnaceScreenHandler handler, Inventory inventory, Component title) {
    ...
    private void renderProgressArrow(GuiGraphicsExtractor context, int x, int y) {
        if(menu.isCrafting()) {
            context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 71, y + 34, 190, 0, menu.getScaledProgress(),15,256,256);
        }
    }
